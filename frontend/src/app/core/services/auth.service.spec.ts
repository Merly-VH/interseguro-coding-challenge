...
describe('AuthService', () => {
  ...
  it('lee config.json y marca configChecked', () => {
  ...
  it('toma la API key de config.json si viene definida (flujo Docker)', () => {
    flushConfig({ apiKey: 'clave-del-despliegue' });

    expect(service.apiKey()).toBe('clave-del-despliegue');
    expect(service.hasConfiguredKey()).toBe(true);
  });

  it('la key manual tiene prioridad sobre la del despliegue', () => {
    flushConfig({ apiKey: 'clave-del-despliegue' });
    service.setApiKey('clave-manual');

    expect(service.apiKey()).toBe('clave-manual');
    expect(service.hasConfiguredKey()).toBe(false);
  });
  ...
  it('pide un token nuevo cuando no hay uno en caché', () => {
    ...
    const req = httpMock.expectOne(`${DEFAULT_API_BASE_URL}/api/token`);
  ...
  it('reutiliza el token en caché mientras no haya expirado', () => {
    ...
    httpMock
      .expectOne(`${DEFAULT_API_BASE_URL}/api/token`)
      .flush({ token: 'jwt-123', expiresIn: 900 });

    let received: string | undefined;
    service.ensureValidToken().subscribe((token) => (received = token));

    httpMock.expectNone(`${DEFAULT_API_BASE_URL}/api/token`);
    expect(received).toBe('jwt-123');
  });
  ...
  it('pide un token nuevo si cambia la API key', () => {
    ...
    httpMock
      .expectOne(`${DEFAULT_API_BASE_URL}/api/token`)
      .flush({ token: 'jwt-1', expiresIn: 900 });
